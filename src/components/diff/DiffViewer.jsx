...
const DiffViewer = ({ diffData, rawContent }) => {
  ...
  const { splitLines, additions, deletions } = useMemo(() => {
    if (!diffData) return { splitLines: [], additions: 0, deletions: 0 };

    let diffLines = diffData.diff || [];
    
    // Handle initial commit/no parent case
    if (diffData.has_parent === false && diffLines.length === 0 && diffData.new_content) {
    ...
    // We need to align deletions and insertions that happen consecutively
    const pairs = [];
    let i = 0;
    ...
    while (i < diffLines.length) {
      ...
      if (current.type === "equal") {
        oNum++;
        nNum++;
        pairs.push({ left: { ...current, num: oNum }, right: { ...current, num: nNum } });
        i++;
      } else if (current.type === "delete") {
        dels++;
        oNum++;
        // Check if the NEXT line is an insert to pair them up side-by-side
        const next = diffLines[i + 1];
        if (next && next.type === "insert") {
          adds++;
          nNum++;
          pairs.push({ left: { ...current, num: oNum }, right: { ...next, num: nNum } });
          i += 2;
        } else {
          pairs.push({ left: { ...current, num: oNum }, right: { type: "empty" } });
          i++;
        }
      } else if (current.type === "insert") {
        adds++;
        nNum++;
        pairs.push({ left: { type: "empty" }, right: { ...current, num: nNum } });
        i++;
      }
    }

    return { splitLines: pairs, additions: adds, deletions: dels };
  }, [diffData]);
      ...
      {showUnsupportedFormat ? (
        ...
        <>
          <div className="flex items-center gap-5 text-[10px] font-black uppercase tracking-widest shrink-0 pb-2.5 mb-2.5 border-b border-base-300/20">
            <span className="flex items-center gap-1.5 text-success"><Plus size={11} /> {additions} Additions</span>
            <span className="flex items-center gap-1.5 text-error"><Minus size={11} /> {deletions} Deletions</span>
          </div>
